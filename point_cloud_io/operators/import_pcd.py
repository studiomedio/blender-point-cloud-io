...
from ..formats.pcd import import_pcd_file


class IMPORT_OT_pcd(bpy.types.Operator, ImportHelper):
    """Import a PCD (Point Cloud Data) file as a PointCloud object"""

    bl_idname = "import_scene.point_cloud_pcd"
    bl_label = "Import PCD"
    ...
    import_colors: BoolProperty(
        name="Colors",
        description="Import the packed RGB / RGBA field if present",
        default=True,
    )
    import_normals: BoolProperty(
        name="Normals",
        description="Import normal_x / normal_y / normal_z if present",
        default=True,
    )
    ...
    def execute(self, context):
        try:
            ...
            self.report({'ERROR'}, f"PCD import failed: {err}")
            return {'CANCELLED'}
...
def menu_func_import_pcd(self, context):
    self.layout.operator(IMPORT_OT_pcd.bl_idname, text="PCD Point Cloud (.pcd)")
